...
| **セキュリティ設計済み** | OWASP Top 10 対策・CSP ヘッダー・スライディングウィンドウ式レート制限・SQL インジェクション対策（Drizzle ORM）を標準装備 |
...
| ミニマル・テック | 力強い・コーポレート | 温かみ・ヒューマン |

> **スクリーンショットは差し替え予定のプレースホルダーです。**
...
[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/AobaIwaki/aoba-lp&env=DATABASE_URL,DATABASE_URL_UNPOOLED,UPSTASH_REDIS_REST_URL,UPSTASH_REDIS_REST_TOKEN,RESEND_API_KEY,RESEND_FROM,ADMIN_EMAIL,SLACK_WEBHOOK_URL)

Vercel のデプロイ画面で環境変数を入力するだけで本番環境が立ち上がります。
...
| [セキュリティ設計書](docs/security.md) | 脅威モデル、OWASP 対策、レート制限実装 |
